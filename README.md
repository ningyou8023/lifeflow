...
LifeFlow 是一款集物品管理、情绪记录、专注辅助、隐私存储、订阅管理于一体的个人生活辅助工具。LifeFlow 迎来 v1.0.54 全新升级，UI/UX 全面焕新！采用现代化的 Material Design 3 设计语言，界面精致优雅，交互流畅自然。新增**日历待办**、**消费追踪**等实用功能，优化核心模块布局，旨在通过极简的美学和隐私优先的原则，帮助用户更好地管理生活琐事。

## ✨ 核心功能 (Features)

LifeFlow 包含以下六个核心模块：

### 1. 🗓️ 日历待办 (Calendar Todo)
*   **待办管理**：在日历视图中直观管理每日任务，支持添加时间与备注。
*   **精准通知**：支持设置任务提醒，到点准时发送通知，不错过重要事项。
*   **桌面小组件**：提供待办事项桌面小组件，直接在桌面查看今日任务。
*   **历史回顾**：方便地查看过去日期的完成情况。

### 2. 🍎 物品与保质期管理 (Pantry & Expiry)
*   **物品录入**：记录物品名称、过期日期、分类。
*   **搜索与分类**：支持多维度分类筛选与关键词搜索，支持**自定义分类顺序**。
*   **智能提醒**：通过颜色标记（红/黄/绿）直观展示过期状态，并在过期前 7天、3天、1天发送本地通知。
*   **AI 智能识物**：支持通过**拍照/相册**识别物品名称，自动填充信息。
*   **消费追踪**：新增**消费追踪**页面，计算物品从购买日至今的日均消费，助您理性消费。
*   **智能菜谱**：AI 根据冰箱里的库存食材，为您推荐美味食谱。

### 3. 📝 AI 情绪日记 (AI Mood Journal)
*   **情绪记录**：记录每日心情指数（1-5星）和文字日记，支持**添加多张图片**。
*   **多分类支持**：支持为日记添加自定义分类标签。
*   **日历视图**：以月度日历形式直观回顾心情轨迹与生活点滴。
*   **AI 分析**：内置 AI 接口，分析日记内容并提供情感建议。新增**深度分析**（情绪趋势与行为相关性）。
*   **桌面小组件**：新增日记小组件，点击即可快速进入写日记页面。
*   **数据导出**：支持将日记导出为 **Markdown** 格式。

### 4. 🧘 专注白噪音 (Focus Mixer)
*   **白噪音播放**：提供雨声、雷声、风声等多种高质量白噪音。
*   **多轨混合**：支持同时播放多种声音并独立调节音量。
*   **专注模式**：支持**番茄时钟**（倒计时）和**正计时**模式，满足不同场景需求。
*   **任务关联**：专注开始前可关联具体任务，记录时间去向。
*   **历史统计**：详细的专注时长统计与图表分析，复盘您的专注力。
*   **后台播放**：支持锁屏后后台继续播放。

### 5. 🔐 隐私保险箱 (Secure Wallet)
...
### 7. 🛠️ 系统与体验 (System & Experience)
*   **国际化**：支持简体中文、繁体中文和英语，满足全球化使用需求。
*   **数据备份**：支持全局数据的加密导出与导入，**包含设置数据**，轻松迁移数据。
*   **桌面小组件**：提供多种样式的小组件（日历待办、AI 日记、过期订阅等）。
*   **隐私保护**：新增隐私政策弹窗，透明展示隐私条款；设置页可随时查看。
*   **导航栏定制**：底部导航栏支持自定义隐藏，首页卡片可快捷跳转至隐藏模块。
...
| 日历待办 (Calendar) | 消费追踪 (Cost) |
|:---:|:---:|
| ![Calendar](screenshots/calendar.png) | ![Cost](screenshots/item_cost.png) |
